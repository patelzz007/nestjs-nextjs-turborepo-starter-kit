...
function RowStarToggle({ request, onChanged }: { readonly request: RequestLogSummary; readonly onChanged: () => void }): React.JSX.Element {
	const { api } = useAuth();
	const starMutation = api.telescope.setAnnotation.useMutation();
...
function RequestsContent(): React.JSX.Element {
	...
	const listQuery = api.telescope.requests.useQuery(query, { placeholderData: (previous) => previous });
