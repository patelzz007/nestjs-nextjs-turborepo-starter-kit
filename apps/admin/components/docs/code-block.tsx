				...
				<div className="flex shrink-0 items-center gap-0.5">
					{showDownloadButton ? (
						<Button
							...
							className="text-muted-foreground/60 hover:bg-muted hover:text-foreground"
					...
					<Button
						...
						className={cn("hover:bg-muted hover:text-foreground", wrapped ? "text-sky-300" : "text-muted-foreground/60")}>
			...
			<div className={cn("relative", isCollapsed && "max-h-[48rem] overflow-hidden")}>
					...
					{isDiff ? (
						...
						<div className="flex">
							...
							<pre
								...
								{code.split("\n").map((line, index) => (
									<span
										key={`${lang}-${String(index)}-${String(line.length)}`}
										className={cn("block px-4", wrapped ? "break-words whitespace-pre-wrap" : "whitespace-pre", diffLineClasses(line))}>
										{line.length === 0 ? " " : line}
									</span>
								))}
							</pre>
				...
				{isLong ? (
					<Button
						...
						className={cn(
							"h-auto w-full justify-center p-0 text-xs font-medium text-sky-300 no-underline transition-colors hover:text-sky-200 hover:no-underline",
