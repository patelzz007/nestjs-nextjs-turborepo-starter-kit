...
export interface SidebarMenuSearchProps {
...
/** Inset sidebar search — soft rail, no heavy border, `/` hint when empty. */
export function SidebarMenuSearch({ value, placeholder, ariaLabel, clearAriaLabel, onChange, onClear, inputRef }: SidebarMenuSearchProps): React.JSX.Element {
	// Local state keeps the input snappy — the parent's debounced
	// `searchQuery` may lag behind the keystroke by up to 150 ms.
	const [localValue, setLocalValue] = React.useState(value);
	const prevValueRef = React.useRef(value);

	// Sync when the parent resets (e.g. clear button, route change).
	// Uses a ref + effect to avoid setState during render.
			...
			<div className="relative">
				...
				{hasQuery ? (
					<button
						type="button"
						onClick={onClear}
						className="absolute top-1/2 right-2.5 -translate-y-1/2 rounded-md p-0.5 text-muted-foreground/70 transition-colors hover:bg-sidebar-accent hover:text-sidebar-foreground"
