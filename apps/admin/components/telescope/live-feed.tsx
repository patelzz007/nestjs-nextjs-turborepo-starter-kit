...
// animated, scrollable list (framer-motion entrance). Request rows show
// method/path/status/duration; exception rows show name/message. Dumb: the
// events + a navigate callback arrive via props.
// ============================================
...
import { durationLabel, statusTone, timeAgo } from "@/lib/telescope";

import type { LiveFeedEvent } from "@/lib/use-telescope-live";

export interface LiveFeedProps {
	readonly events: readonly LiveFeedEvent[];
	/** Called when a feed row is clicked (routes to detail/exceptions). */
...
function FeedRow({ event, onNavigate }: FeedRowProps): React.JSX.Element {
	const isException: boolean = event.type === "exception";
	const tone = statusTone(event.statusCode ?? null);
		...
		return (
			<motion.button
				...
				<span className="min-w-0 flex-1">
					<span className="block truncate font-mono text-xs font-medium text-destructive">{event.name ?? "Exception"}</span>
