 ...
 */
export function StatCard({ label, value, sub, icon, accentClass = "text-primary", href }: StatCardProps): React.JSX.Element {
	const body: React.JSX.Element = (
		<CardContent className="flex items-start justify-between gap-3 p-4">
			<div className="min-w-0">
				<p className="truncate text-xs font-medium tracking-wide text-muted-foreground uppercase">{label}</p>
				<p className="mt-1.5 truncate text-2xl font-semibold tracking-tight text-foreground tabular-nums">{value}</p>
				{sub !== undefined ? <p className="mt-1 truncate text-xs text-muted-foreground">{sub}</p> : null}
			</div>
			{icon !== undefined ? <div className={cn("flex size-9 shrink-0 items-center justify-center rounded-lg bg-muted/60", accentClass)}>{icon}</div> : null}
		</CardContent>
	);
