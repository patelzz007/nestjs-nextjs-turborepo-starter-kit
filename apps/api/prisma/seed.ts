...
import { PrismaClient, Permission, Role, User, Url, Tag, MenuItem, PermissionAction, PermissionResource, DeviceType, RedirectType, Plan } from "@prisma/client";
...
const COUNTRIES = ["MY", "US", "GB", "SG", "AU", "IN", "DE", "JP", "CA", "FR", "BR", "KR"];
const CITIES = ["Kuala Lumpur", "New York", "London", "Singapore", "Sydney", "Mumbai", "Berlin", "Tokyo", "Toronto", "Paris", "São Paulo", "Seoul"];
const DEVICES: DeviceType[] = ["DESKTOP", "DESKTOP", "MOBILE", "MOBILE", "MOBILE", "TABLET", "BOT"];
const OSS = ["Windows", "macOS", "Android", "iOS", "Linux", "Ubuntu", "ChromeOS"];
const BROWSERS = ["Chrome", "Safari", "Firefox", "Edge", "Samsung Internet", "Opera", "Brave"];
const REFERRERS = [
	"https://google.com",
	"https://twitter.com",
	"https://facebook.com",
...
const UTM_SOURCES = ["google", "facebook", "twitter", "linkedin", "email", "newsletter"];
...
async function assignPermissionsToRoles(roles: Role[], permissions: Permission[]): Promise<void> {
...
async function assignAdditionalPermissions(users: User[], permissions: Permission[]): Promise<void> {
	const get = (email: string) => users.find((u) => u.email === email)!;
...
async function createUrlTags(users: User[], urls: Url[], tags: Tag[]): Promise<void> {
...
async function createMenuItems(permissions: Permission[], roles: Role[]): Promise<void> {
	const perm = (action: PermissionAction, resource: PermissionResource) => permissions.find((p) => p.action === action && p.resource === resource)!;
	const role = (name: string) => roles.find((r) => r.name === name)!;
...
async function generateAdditionalSeedData(roles: Role[], userRole: Role): Promise<User[]> {
	const hash = (pw: string): Promise<string> => bcrypt.hash(pw, 10);
