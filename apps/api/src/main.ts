...
async function bootstrap(): Promise<void> {
	...
	let phaseStart: number = bootStart;
	const heapUsed = (): string => (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(1);
	...
	// On by default; set SWAGGER_ENABLED=0 to skip OpenAPI + /v1/docs in dev.
	const swaggerEnabled: boolean = process.env.SWAGGER_ENABLED !== "0";

	// Validate environment variables
	const apiEnvResult = validateApiEnv(process.env);
	if (!apiEnvResult.success) {
		console.error("Environment validation failed:");
		console.error(apiEnvResult.error);
		process.exit(1);
	}
	...
	const listenHost: string = process.env.HOST ?? "127.0.0.1";
	const corsOrigins: string[] = (process.env.CORS_ORIGINS ?? "http://localhost:3000,http://localhost:3001,http://localhost:3003")
		.split(",")
	...
	// Set SWAGGER_ENABLED=0 to skip in dev (saves ~50ms + static UI mount on listen).
	if (swaggerEnabled) {
