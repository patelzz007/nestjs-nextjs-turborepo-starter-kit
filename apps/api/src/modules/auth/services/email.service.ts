...
export class EmailService {
	...
	public async sendPasswordResetEmail(email: string, resetToken: string): Promise<void> {
		const template = new PasswordResetEmailTemplate({ to: email, resetToken, expiresInHours: 1 });
	...
	public async sendVerificationEmail(email: string, verificationToken: string): Promise<void> {
		const template = new VerificationEmailTemplate({ to: email, verificationToken, expiresInHours: 24 });
		await this.sender.send(template);
	}
