...
| `EADDRINUSE: :::8080`                                                                                                                      | A previous API process is still running (incl. an orphaned dev server from `pnpm dev`)                                                                                                                                                                                                                | `pnpm kill:port` (kills whatever holds 8080), then re-run `pnpm run dev`. Check `lsof -nP -i :8080 -sTCP:LISTEN` if it recurs — kill the whole process tree, not just the child.                                                                                                                                                                                                                                                                                              |
