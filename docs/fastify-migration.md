...
| 10 | `src/modules/telescope/telescope-capture.middleware.ts` | `req.originalUrl`, `req.method`, **`req.body`** | **⚠ `req.body` is `undefined` in middie middleware** (runs at `onRequest`, before body parsing) — needs a bridge hook (see §6) |
...
| 14 | `src/modules/telescope/*` (interceptor, admin guard) | `Request` types | `FastifyRequest` |
...
| 19 | `src/modules/telescope/telescope.controller.ts` | `@Sse("stream")` | Verify SSE on Fastify (§7) |
...
The current consumers (`?limit=`, `?page=`, `?client_type=`, telescope filters)
...
Fastify it will be `undefined`, so telescope's request-body capture silently
becomes `null`.

**Fix — a tiny body-bridge `preHandler` hook** (registered once in `main.ts`
or the telescope module) that mirrors the parsed body onto the raw request so
...
**Acceptance:** after migration, open a telescope request detail page — request
...
### 7.2 SSE streams (`email-log.controller.ts`, `telescope.controller.ts`)
...
1. Reconnect semantics — the telescope client sends `Last-Event-ID`; confirm
...
**Acceptance:** open `/telescope` in the admin → live feed updates on new
...
## 9. Phase 7 — Build pipeline (SWC + esbuild)

The current pipeline is `nest build` (SWC) → `scripts/bundle-prod.mjs`
(esbuild). Verification checklist — no changes expected, just confirm:

1. **SWC build** (`nest build`) — `@nestjs/platform-fastify` is CJS; the SWC
   output must still emit `design:paramtypes` for DI. Boot `tsx dist/src/main.js`.
2. **esbuild bundle** (`pnpm bundle`) — the bundle keeps bare packages external;
   Fastify plugins (`@fastify/cors`, `@fastify/cookie`) resolve from
   `node_modules` at runtime. Boot `node dist/main.bundle.js` and hit `/health`,
   `/auth/login` (401 without cookies), and one SSE endpoint.
3. **Watch mode** (`nest start --watch -e tsx`) — hot reload on a changed
   controller.
...
|------|-----------|------------|
| `req.body` missing in telescope middleware | **Certain** | §6.2 bridge hook |
...
| esbuild bundle breaks on Fastify lazy `import('@fastify/cors')` | Low | §9 external list |
