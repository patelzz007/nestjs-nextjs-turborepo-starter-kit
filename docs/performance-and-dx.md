...
| 2   | Rspack prod bundle (NestJS v12)           | ⚡ Speed | —        | S      | ✅     |
...
### 2. Rspack prod bundle — ✅ done

**Status:** ✅ Done. The API compiles with **Rspack** (`rspack.config.mjs`) using
built-in `swc-loader` for legacy decorators + `emitDecoratorMetadata`.
Production builds minify (with class/function names preserved for Nest metadata)
and clean `dist/` first.
...
@workspace/api start`), polls `/health` up to N seconds, asserts 200 + JSON
