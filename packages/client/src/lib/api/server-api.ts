...
// with `.query()` (direct, parsed). The REST transport is the same registry
...
// and the RxJS retry/refresh pipeline, so there is no `unknown`, no type
// assertion, and no eslint-disable anywhere.
import "server-only";
...
// ── The tRPC-style server caller ───────────────────────────────────────────/** One GET leaf on the caller: callable as `.query()` for direct server-side fetching. */
...
/** Builds the caller leaf for one GET def — direct `.query()` server-side fetching. */
