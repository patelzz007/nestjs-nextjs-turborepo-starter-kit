...
function Alert({
	...
	const trailing = (
		<div className="flex shrink-0 items-center gap-1.5">
			{details !== undefined ? (
				<Button type="button" variant="ghost" size="icon-xs" data-slot="alert-copy" aria-label="Copy details" className="size-6 text-muted-foreground" onClick={handleCopy}>
	...
	if (modal) {
		return (
			<div className="fixed inset-0 z-overlay grid place-items-center bg-black/40 p-4 backdrop-blur-xs" role="presentation">
				{rootContent}
			</div>
		);
	}
...
function AlertGroup({ className, floating = false, ...props }: AlertGroupProps): React.JSX.Element {
	return (
		<div
			...
			className={cn("flex flex-col gap-2", floating && "fixed inset-x-4 bottom-4 z-overlay sm:inset-x-auto sm:inset-e-4 sm:w-full sm:max-w-sm", className)}
			{...props}
		/>
	);
}
