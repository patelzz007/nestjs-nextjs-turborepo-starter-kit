...
function SidebarProvider({
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)): void => {
			...
			document.cookie = `${SIDEBAR_COOKIE_NAME}=${String(nextOpen)}; path=/; max-age=${String(SIDEBAR_COOKIE_MAX_AGE)}`;
		},
		[setOpenProp, open],
	);
...
function Sidebar({
	...
	if (isMobile) {
		const mobileSidebarStyle: React.CSSProperties & Record<`--${string}`, string> = {
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>): React.JSX.Element {
	...
	return (
		<Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon-sm" className={cn(className)} onClick={handleSidebarTriggerClick} {...props}>
...
function SidebarMenuSubButton({
	...
	// Height deliberately matches SidebarMenuButton's default `h-8` so nested
	// items stay the same size as top-level items at every depth. Don't shrink
	// this back to `h-7` — it makes child rows look progressively smaller.
	return useRender({
		defaultTagName: "a",
		props: mergeProps<"a">(
			{
				className: cn(
					"flex h-8 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden group-data-[collapsible=icon]:hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs rtl:translate-x-px data-active:bg-slate-800 data-active:font-medium data-active:text-white data-active:hover:bg-slate-800! data-active:hover:text-white! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground data-active:[&>svg]:text-white",
					className,
				),
			},
			props,
